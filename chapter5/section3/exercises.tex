...
\begin{enumerate} [label=\textbf{\arabic*.}, leftmargin=0em]
    ...
    \item Let $X$ be a nonsingular projective variety of any dimension, let $Y$ be a nonsingular subvariety, and let $\pi : \tilde{X} \to X$ be obtained by blowing up $Y$.
          ...
          \begin{proof}
              The proof of (3.4) follows through the more general case by (II, 8.24).
    ...
    \item Let $C$ and $D$ be curves on a surface $X$, meeting at a point $P$.
          ...
          \begin{proof}
            Let $E$ be the exceptional locus of $\pi : \tilde{X} \to X$.
            By (3.6), we have
            \begin{equation*}
                \pi^*C = \tilde{C} + \mu_P(C)E.
            \end{equation*}
            Hence, we have
            \begin{equation*}
                \tilde{C}.\tilde{D} = (\pi^*C - \mu_P(C)E).(\pi^*D - \mu_P(D)E) = C.D - \mu_P(C) \cdot \mu_P(D).
            \end{equation*}

            Assuming $C$ and $D$ are transversal, meeting at a finite number of points, the strict transforms of $C$ and $D$ under the blowing up of all intersection points will not meet, i.e., $\tilde{C}.\tilde{D} = 0$.
            Each time an intersection point is blown up, $C.D$ goes does by the product of the intersection multiplicities of each curve. 
          \end{proof}
    ...
    \item[\textbf{5.}] Let $a_1, \dots, a_r$, $r \geq 5$, be distinct elements of $k$, and let $C$ be the curve in $\P^2$ given by the (affine) equation $y^2 = \prod_{i = 1}^r (x - a_i)$.
    ...
    \item[\textbf{7.}] For each of the following singularities at $(0, 0)$ in the plane, give an embedded resolution, compute $\delta_P$, and decide which ones are equivalent.
          \begin{itemize}
              \item[(a)] $x^3 + y^5 = 0$
