...
\begin{enumerate} [label=\textbf{\arabic*.}, leftmargin=0em]
    ...
    \item If $X$ is a birationally ruled surface, show that the curve $C$, such that $X$ is birationally equivalent to $C \times \PP^1$, is unique up to isomorphism.

    \item Let $X$ be the ruled surface $\PP(\fE)$ over a curve $C$.
    ...
    \item \begin{itemize}
              ...
              \item[(b)] Show that this is false for varieties of dimension $\geq 2$.
                    In particular, the sheaf of differentials $\Omega_{\PP^2}$ on $\PP^2$ is not an extension of invertible sheaves.
          \end{itemize}

    \item Let $C$ be a curve of genus $g$, and let $X$ be the ruled surface $C \times \PP^1$.
    ...
    \item Show that every locally free sheaf of finite rank on $\PP^1$ is isomorphic to a direct sum of invertible sheaves.

    \item On the elliptic ruled surface $X$ of (2.11.6), show that the sections $C_0$ with $C_0^2 = 1$ form a one-dimensional algebraic family, parametrized by the points of th ebase curve $C$, and that no tow are linearly equivalent.
    ...
    \item Let $Y$ be a nonsingular curve on a quadric cone $X_0$ in $\PP^3$.
    ...
    \item For any $n > e \geq 0$, let $X$ be the rational scroll of degree $d = 2n - e$ in $\PP^{d + 1}$ given by (2.19).
    ...
    \item Let $X$ be a ruled surface over the curve $C$, defined by a normalized bundle $\fE$, and let $\goth{e}$ be the divisor on $C$ for which $\fO_C(\goth{e}) \cong \wedge^2 \fE$ (2.8.1).
          Let $\goth{b}$ be any divisor on $C$.
          \begin{itemize}
              \item[(a)] If $|\goth{b}|$ and $|\goth{b} + \goth{e}$ have no base points, and if $\goth{b}$ is nonspecial, then there is a section $D \sim C_0 + \goth{b}f$, and $|D|$ has no points.
              \item[(b)] If $\goth{b}$ and $\goth{b} + \goth{e}$ are very ample on $C$, and for every point $P \in C$, we have $\goth{b} - P$ and $\goth{b} + \goth{e} - P$ nonspecial, then $C_0 + \goth{b}f$ is very ample.
          \end{itemize}

    \item Let $X$ be a ruled surface with invariant $e$ over an elliptic curve $C$, and let $\goth{b}$ be a divisor on $C$.
          \begin{itemize}
              \item[(a)] If $\deg{\goth{b}} \geq \goth{e} + 2$, then there is a section $D \sim C_0 + \goth{b}f$ such that $|D|$ has no base points.
              \item[(b)] The linear system $|C_0 + \goth{b}f|$ is very ample if and only if $\deg{\goth{b}} \geq e + 3$.
          \end{itemize}

    \item For every $e \geq -1$ and $n \geq e + 3$, there i san elliptic scroll of degree $d = 2n - e$ in $\PP^{d _ 1}$.
          In particular, there is an elliptic scroll of degree 5 in $\PP^4$.

    \item Let $X$ be a ruled surface over a curve of genus $g$, with invariant $e < 0$, and assume that $\chr{k} = p > 0$ and $g \geq 2$.
          \begin{itemize}
    ...
    \item \begin{itemize}
              \item[(a)] Let $\varphi : \PP_k^1 \to \PP_k^3$ be the 3-uple embedding.
                    Let $\fI$ be the sheaf of ideals of the twisted cubic curve $C$ which is the image of $\varphi$.
                    Then $\fI / \fI^2$ is a locally free sheaf of rank 2 on $C$, so $\varphi^*(\fI / \fI^2)$ is a locally free sheaf of rank 2 on $\PP^1$.
                    By (2.14), therefore, $\varphi^*(\fI/\fI^2) \cong \fO(l) \oplus \fO(m)$ for some $l, m \in \Z$.
                    Determine $l$ and $m$.
              \item[(b)] Repeat part (a) for the embedding $\varphi : \PP^1 \to \PP^3$ given by $x_0 = t^4, x_1 = t^3u, x_2 = tu^3, x_3 = u^4$, whose image is a nonsingular rational quartic curve.
          \end{itemize}
