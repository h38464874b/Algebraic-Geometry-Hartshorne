...
%     \item[(d)] For any variety $X$, show that $\Hom{(X, \G_a)}$ is isomoprhic to $\fO(X)$ as a group under addition.
%     \item[(e)] For any variety $X$, show that $\Hom{(X, \G_m)}$ is isomorphic to the group of units in $\fO(X)$, under multiplication.
% \end{itemize}
