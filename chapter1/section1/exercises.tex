...
% \item 
% \begin{itemize}
%     \item[(a)] Let $Y$ be the plane curve $y = x^2$ (i.e. $Y$ is the zero set of the polynomial $f = y - x^2$).
%     Show that $A(Y)$ is isomorphic to a polynomial ring in one variable over $k$.

%     \item[(b)] Let $Z$ be the plane curve $xy = 1$.
%     Show that $A(Z)$ is not isomorphic to a polynomial ring in one variable over $k$.

%     \item[(c)] Let $f$ be any irreducible quadratic polynomial in $k[x, y]$, and let $W$ be the conic defined by $f$.
...
% \item Give an example of an irreducible polynomial $f \in \R[x, y]$ whose zero set $Z(f)$ in $\A_\R^2$ is not irreducible.
