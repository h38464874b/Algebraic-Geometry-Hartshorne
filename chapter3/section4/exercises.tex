...
By hypothesis, the function field of $X$ is a finite field extension of that of $Y$, so there exists an isomorphism $\phi : \fK^r \to f_*\fL$ for some $r > 0$.
Let $\alpha : \fO_Y^r \to f_*\fL$ be the compositon of the natural map $\fO_Y^r \to \fK^r$ with $\phi$. 
Then $\alpha$ is an ismorphism at the generic point of $Y$.
Replacing $\fL$ by the sub-$\fO_X$-module spanned by the image of $\fK^r$, we obtain the desired $\fM$.

\item Let $\fF$ be any coherent sheaf $\fF$ on $Y$, let $\fA = f_*\fO_X$, and let $\alpha : \fO_Y^r \to f_* \fM$ as in (a) for some coherent sheaf $\fM$ on $X$.
Applying $\rHom(\cdot, \fF)$ induces a morphism of sheaves
\begin{equation*}
  \beta : \rHom(f_*\fM, \fF) \to \rHom(\fO_Y^r, \fF) \cong \fF^r.
\end{equation*}
It is clearly an isomorphism at the generic point of $Y$.
Consider the $\fO_Y$-module $\rHom(f_*\fM, \fF)$.
It is naturally a coherent $\fA$-module, and finite morpshims are also affine, so by the equivalence of categories of quasi-coherent $\fO_X$-modules and $\fA$-modules via the $\widetilde{}~$-functor of (II, 5.17e), there exists a coherent sheaf $\fG$ on $X$ such that $f_*\fG \cong \rHom(f_*\fM, \fF)$.

\item By (II, 3.1), (Ex. 3.1), and (Ex. 3.2), we reduce to the case when $X$ and $Y$ are integral Noetherian separated schemes.
Let $\fF$ be a coherent sheaf on $Y$.
By (2.9) and (3.7), it suffices to show $H^i(Y, \fF^r) = 0$ for all $i > 0$ and some $r > 0$.
Let $\beta : f_*\fG \to \fF^r$ be a morphism of coherent sheaves on $X$ as in (b), where $\fG$ is a coherent sheaf on $X$.

Consider $\ker{\beta}$ and $\coker{\beta}$.
We proceed by notherian induction
