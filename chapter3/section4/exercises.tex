...
\item Let $V_i = \spec{A_i}$ be a finite open affine cover of $Y$ such that $U_i = f^{-1}(V_i)$ is affine, equal to the spectra of an $A_i$-algebra $B_i$ that is finitely generated as an $A_i$-module.
Note that $Y$ integral and $f$ surjective implies the associated ring homomorphisms $A_i \to B_i$ are injective.
In particular, the function field of $Y$ is a subfield of that of $X$.
