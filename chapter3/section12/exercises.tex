...
\begin{enumerate} [label=\textbf{\arabic*.}, leftmargin=0em]
  ...
  \item Let $\{X_t\}$ be a family of hypersurfaces of the same degree in $\P_k^n$. Show that for each $i$, the function $h^i(X_t, \fO_{X_t})$ is a constant function of $t$.

        \begin{proof}
          Hypersurfaces of same degree in projective space $\P^n$ have the same cohomology.
          For any hypersurface $X \subset \P^n$ of degree $d$, we have an exact sequence
          \[ \begin{tikzcd}
              0 \arrow[r] & \fO_{\P^n}(-d) \arrow[r] & \fO_{\P^n} \arrow[r] & \fO_X \arrow[r] & 0,
            \end{tikzcd} \]
  ...
  \item[\textbf{5.}] Let $Y$ be an integral scheme of finite type over an algebraically closed field $k$. Let $\mathscr{E}$ be a locally free sheaf on $Y$, and let $X = \P(\fE)$. Then show that $\pic{X} \cong (\pic{Y}) \times \Z$. This strengthens (II, Ex. 7.9).
        ...
        \begin{proof}
          ...
          \end{align*}
          Since $\pi$ is a flat projective morphism, we can apply the results of \SS 12.
          In particular, if $\fM$ is an invertible sheaf on $X$, then $\fM_y$ is an invertible sheaf on $X_y \cong \P_y^n$ for all $y \in Y$, where $\P_y^n$ is the projective $n$-space over $k(y)$.
          In particular, we can write $\fM_y \cong \fO_{\P^n_y}(d_y)$ for some $d_y \in \Z$.
          ...
          \begin{equation*}
            \cdh^0(y, \fM) = \dim_{k(y)} \cH^0(\P^n_y, \fO_{\P^n_y}(d_y))
          \end{equation*}
          is upper semicontinuous on $Y$ (12.8), and we have the strict inequality
          \begin{equation*}
            \dim_{k(y')} \cH^0(\P_{y'}^n, \fO_{\P_{k(y)}}) < \dim_{k(\xi)} \cH^0(\P_{\xi}^n, \fO_{\P^n_{\xi}}(d_\xi))
          \end{equation*}
          ...
          \begin{align*}
            h^n(y, \fM) & = \dim_{k(y)} \cH^n(\P_y^n, \fO_{\P_y^n}(d_y))          \\
                        & = \dim_{k(y)} \cH^n(\P_y^n, \fO_{\P_y^n}(-d_y - n - 1)) \\
          ...
          \begin{equation*}
            \dim_{k(y')} \cH^0(\P_{y'}^n, \fO_{\P_{y'}^n}(-d_{y'} - n - 1)) < \dim_{k(\xi)} \cH^0(\P_\xi^n, \fO_{\P_\xi^n}),
          \end{equation*}
...
% \item Let $X_1 \subseteq \P_k^4$ be the \textit{rational normal quartic curve} (which is the $4$-uple embedding of $\P^1$ in $\P^4$). Let $X_0 \subseteq \P^3$ be a nonsingular rational quartic curve, such as the one in (I, Ex. 3.18b). Use (9.8.3) to construct a flat family $\{X_t\}$ of curves in $\P^4$, parameterized by $T = \A^1$, with the given fibers $X_1$ and $X_0$ for $t = 1$ and $t = 0$.
% 
% Let $\fI \subseteq \fO_{\P^4 \times T}$ be the ideal sheaf of the total family $X \subseteq \P^4 \times T$. Show that $\fI$ is flat over $T$. Then show that
% \begin{equation*}
...
% 
% \begin{proof}
%   Suppose $X_1$ is given by the standard 4-uple embedding $\P^1$ in $\P^4$ (I, Ex. 2.12), and $X_0$ is given by (I, Ex. 3.18b).
%   Then, the point $P = (0 : 0 : 1 : 0 : 0)$ does not lie in $X_1$, so by applying (9.8.3)  to the projection $\P^{4} - P \to \P^3$ and automorphisms $(x_0 : x_1 : x_2 : x_3 : x_4) \mapsto (x_0 : x_1 : ax_2 : x_3 : x_4)$ for each $a \neq 0$, we have our desired flat family $\{ X_t\}$ of curves in $\P^4$.
...
%   We proceed by going back to projective space $\P^3$ and introducing another homogenous coordinate $v$.
...
% 
%   By construction, $X_t$ for every $t \neq 0$ is isomorphic to $X_1$, so to compute $h^i(t, \fI)$ for $t \neq 0$, it suffices to compute $\cH^i(\P^4, \fI_1)$.
%   The exact sequence of $\fO_{\P^4}$-modules
%   \[ \begin{tikzcd}
%     0 \arrow[r] & \fI_1 \arrow[r] & \fO_{\P^4} \arrow[r] & \fO_{X_1} \arrow[r] & 0
%   \end{tikzcd} \]
%   induces an exact sequence in cohomology
%   \[ \begin{tikzcd}
% 0 \arrow[r]  & \cH^0(\fI_1) \arrow[r] & \cH^0(\fO_{\P^4}) \arrow[r] & \cH^0(\fO_{X_1}) \arrow[r] & {} \\
% {} \arrow[r] & \cH^1(\fI_1) \arrow[r] & \cH^1(\fO_{\P^4})           &                            &   
% \end{tikzcd}\]
%   Since $X_1$ and $\P^4$ are rational, we have the explicit calculations of (5.1)
%   \begin{equation*}
%     h^0(\fO_{\P^4}) = 1, \quad h^1(\fO_{X_1}) = 1, \quad h^1(\fO_{\P^4}) = 0,
%   \end{equation*}
%   and since $\cH^0(\fO_{\P^4}) \to \cH^1(\fO_{X_1})$ is surjective, by exactness we conclude that $h^i(\fI) = 0$ for $i = 0, 1$.
% \end{proof}
