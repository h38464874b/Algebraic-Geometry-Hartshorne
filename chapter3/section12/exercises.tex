...
\begin{proof}
  ...
  For each closed point $y \in Y$, we have a natural isomorphism
  \begin{equation*}
...
\begin{proof}
  ...
  \end{tikzcd} \]
  and $h^i(X, \fO_X)$ is independent of the choice of $X$.
\end{proof}
...
\begin{proof}
  ...
  For each $y \in Y$, $(\fL \otimes \fM^{-1})_y \cong \fO_{X_y}$ on the fibre $X_y$, and each $X_y$ is integral and projective over $k(y)$, so we have $$h^0(X_y, (\fL \otimes \fM^{-1})_y) = h^0(X_y, \fO_{X_y}) = 1.$$
...
\begin{proof}
  ...
  \end{align*}
  Since $\pi$ is a flat projective morphism, we can apply the results of this section.
  ...
  \begin{equation*}
    h^0(y, \fM) = \dim_{k(y)} \cH^0(\PP^n_y, \fO_{\PP^n_y}(d_y))
  \end{equation*}
  is upper semicontinuous on $Y$ (12.8), and we have the strict inequality
  \begin{equation*}
    1 = \dim_{k(y')} \cH^0(\PP_{y'}^n, \fO_{\PP_{k(y)}}) < \dim_{k(\xi)} \cH^0(\PP_{\xi}^n, \fO_{\PP^n_{\xi}}(d_\xi))
  \end{equation*}
  ...
  \begin{equation*}
    h^n(y, \fM) = \dim_{k(y)} \cH^n(\PP_y^n, \fO_{\PP_y^n}(d_y)) = \dim_{k(y)} \cH^n(\PP_y^n, \fO_{\PP_y^n}(-d_y - n - 1)) = h^0(y, \fO(-n-1) \otimes \fM^{-1}),
  \end{equation*}
  ...
  \begin{equation*}
     0  = \dim_{k(y')} \cH^0(\PP_{y'}^n, \fO_{\PP_{y'}^n}(-d_{y'} - n - 1)) = h^n(y', \fM) < h^n(\xi, \fM) = \dim_{k(\xi)} \cH^0(\PP_\xi^n, \fO_{\PP_\xi^n}) = 1,
  \end{equation*}
...
\begin{enumerate} [leftmargin=0cm]
  \item[(a)] Let $\fO_X(1)$ be a very ample invertible sheaf on $X$.
