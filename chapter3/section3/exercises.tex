...
\item[(a)] The $\Hom$ functor commutes with finite direct sums in the second argument, so we can assume $\fG = j_* \widetilde{I}$, where $j : U = \spec{A} \to X$ is the inclusion, and $I$ is an injective $A$-module.
Suppose $\fN \to \fM$ is an injective map of $\fO_X$-modules, and we are given any $f : \fN \to j_*\widetilde{I}$.
Since $j^*$ is left exact when $j$ is an open immersion, the induced map of $A$-modules $j^* \fN \to j^* \fM$ is also injective.
For any such $f$ there is an associated morphism of $A$-modules $g : j^* \fN \to \widetilde{I}$ by adjointness of $j_*$, so there exists an extension of $g$ to $j^*\fM$ by injectivity of $\widetilde{I}$.
By adjointness of $j^*$ again, we obtain a morphism $\fM \to j_*\widetilde{I}$ that naturally extends $f$, which is what we wanted to show.

\item[(b)] Essentially imitating (a), we deduce that $\restr{\fI}{U}$ is an injective object of $\goth{Qco}(U)$.
Covering $X$ with finite number of open affines $U_i = \spec{A_i}$, we have $\restr{\fI}{U_i} \cong \widetilde{I}_i$  for some injective $A_i$-module $I_i$ for each $i$ by (II, 5.5).
