...
Show that $X$ is affine if and only if $\red{X}$ (II, Ex. 2.3) is affine.

\begin{proof}
  One direction is clear.
  Suppose $\red{X} = \spec{A}$ where $A$ is a Noetherian ring with no nilpotent elements, let $f : \red{X} \to X$ be the natural map, and let $\fF$ be any quasi-coherent sheaf on $X$. 
  ...
  Note that $X \cong \red{X}$ as topological space, and the associated morphism of sheaves $\fO_X \to f_* \fO_{\red{X}}$ is surjective with kernel $\fN$.
  Thus, each of the quotients of this filtration can be naturally viewed as $A$-modules.
  In particular, we have a natural isomorphism (2.10) $$\cH^i(X, \fN^r \cdot \fF / \fN^{r + 1} \fF) \cong \cH^i(\red{X}, f^*(\fN^r \cdot \fF / \fN^{r + 1} \cdot \fF)).$$
...
For any such $f$ there is an associated morphism of $A$-modules $g : j^* \fN \to \tilde{I}$ by adjointness of $j_*$, so there exists an extension of $g$ to $j^*\fM$ by injectivity of $\tilde{I}$.
...
The map $\Gamma(X, \tilde{I}) \to \Gamma(U, \tilde{I})$ is induced by the natural maps $\Hom_A(A, I) \to \Hom_A(\goth{a}^n, I)$, which is surjective since $I$ is injective.
