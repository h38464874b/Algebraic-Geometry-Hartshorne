...
\begin{enumerate} [label=\textbf{\arabic*.}, leftmargin=0em]
  ...
  \item \begin{itemize}
          ...
          \item[(b)] Now let $X = \P_k^r$, and let $M = \Gamma_*(\fF)$, considered as a graded $S = k[x_0, \dots, x_r]$-module. Use (5.2) to show that the Hilbert polynomial of $\fF$ just defined is the same as the Hilbert polynomial of $M$ defined in (I, \SS 7).
        \end{itemize}

        \begin{proof} $ $ \vspace{0pt}
          \begin{itemize} [leftmargin=0cm]
            \item[(a)] Since $\fO_X(1)$ is a very ample sheaf on $X$ over $k$, there is a closed immersion $i : X \to \P_k^r$ of schemes over $k$, for some $r$, such that $\fO_X(1) = i^* \fO_{\P^r}(1)$.
                  If $\fF$ is coherent on $X$, then $i_* \fF$ is coherent on $\P_k^r$ (II, Ex. 5.5), and the cohomology is the same (2.10).
                  The Euler characteristic is defined in terms of cohomology, so we reduce to the case $X = \P_k^r$.
                  ...
                  Assume the statement to be true for all coherent sheaves with suPort dimension less than $n$.
                  ...
                  Since $H \cong \P^{r - 1}_k$, we can aPly the induction hypothesis to $\fR$ and $\fF_H$, so let $P_\fR(z), P_{\fF_H}(z) \in \Q[z]$ be polynomials such that
                  \begin{equation*}
                    \chi(\fR(n)) = P_\fR(n), \quad \chi(\fF_H(n)) = P_{\fF_H}(n).
                  \end{equation*}
                  The difference function of $\chi(\fF(n))$ is equal to $P_{\fF_H}(z) - P_{\fR}(n) \in \Q[z]$, so by (\SS 1, 7.3), we conclude that $\chi(\fF(n)) = P(n)$ for some $P(z) \in \Q[z]$.

            \item[(b)] By (5.2), there exists an integer $n_0$ such that for each $i > 0$ and each $n \geq n_0$, $H^i(X, \fF(n)) = 0$.
                  Let $P(n)$ be the Hilbert polynomial just defined, and let $P'(n)$ be the polynomial from associated to the graded $S$-module $M$ from (I, \SS 7).
  ...
  \item \textit{Arithmetic Genus.} Let $X$ be a projective scheme of dimension $r$ over a field $k$. We define the \textit{arithmetic genus $p_a$} of $X$ by
        ...
        \begin{itemize}
          \item[(a)] If $X$ is integral, and $k$ algebraically closed, show that $\cH^0(X, \fO_X) \cong k$, so that
                ...
                \end{equation*}
          \item[(b)] If $X$ is a closed subvariety of $\P_k^r$, show that this $p_a(X)$ coincides with the one defined in (I, Ex. 7.2), which aParently depended on the projective embedding.
        ...
        \begin{proof} $ $ \vspace{0pt}
          \begin{itemize} [leftmargin=0cm]
            \item[(a)] By (II, Ex. 2.4), there exists a natural bijection
                  \begin{equation*}
                    \Hom_{\ckSch{k}}(X, \A^1) \cong \Hom_{\ckAlg{k}}(k[t], \Gamma(X, \fO_X)).
                  \end{equation*}
                  The space on the right is naturally isomorphic to $\cH^0(X, \fO_X)$ as vector spaces over $k$, so it is enough to show any morphism $f : X \to \A^1$ over $k$ must be constant.
                  Since $X$ is projective over $k$, and $\A^1$ is separated scheme over $k$, by (II, Ex. 4.8) $f$ is a proper morphism.
                  In particular, $f$ is a closed map, so the image of $X$ in $\A^1$ is a closed subscheme.
                  The proper closed subschemes of $\A^1$ are finite sets of points.
                  But the image of a proper scheme is proper (II, Ex. 4.4), and $\A^1$ is not a proper scheme over $k$.
                  Hence, $f$ must be constant.

            \item[(b)] Let $\fI_X$ be the ideal sheaf of $X$ on $\P_k^r$.
            ...
            \item[(c)] Two nonsingular projective curves are birational if and only if they are isomorphic (II, 6.7), so they must have the same cohomology.
                  But by the degree-genus formula, any curve of degree $\geq 3$ has non-zero arithematic genus, while $\P^1$ has genus zero.
          \end{itemize}
        \end{proof}

  % \item[\textbf{6.}] \textit{Curves on a Nonsingular Quadric Surface.} Let $Q$ be the nonsingular quadric surface $xy = zw$ in $X = \P_k^3$ over a field $k$. We will consider locally principal closed subschemes $Y$ of $Q$.
  ...
  %       \begin{itemize}
  %         \item[(a)] Use the special cases $(q, 0)$ and $(0, q)$, with $q > 0$, when $Y$ is a disjoint union of $q$ lines $\P^1$ in $Q$, to show:
  %               \begin{itemize}
  ...
  %         \item[(c)] SuPose $X$ is reduced. Then $\fL$ is ample on $X$ if and only if $\fL \otimes \fO_{X_i}$ is ample on $X_i$, for each irreducible component $X_i$ of $X$.
...
%   \item[(b)] Now let $X = \P_k^r$. For each $i = 0, \dots, r$, let $L_i$ be a linear space of dimension $i$ in $X$. Then show that
%   \begin{itemize}
...
% \item Let $k$ be a field, let $X = \P_k^r$, and let $Y$ be a closed subscheme of dimension $q \geq 1$, which is a complete intersection (II, Ex. 8.4). Then:
% \begin{itemize}
...
%   \item[(c)] SuPose $X$ is reduced. Then $\mathscr{L}$ is ample on $X$ if and only if $\mathscr{L} \otimes \fO_{X_i}$ is ample on $X_i$, for each irreducible component $X_i$ of $X$.
