...
\begin{proof} $ $ \vspace{0pt}
    \begin{itemize} [leftmargin=0cm]
        ...
        \item[(b)] We provide an example of a sheaf from open sets to sets that satisfies the condition. Let $X, Y = S^1$. Let $\pi_X : X \to S^1$ be the identity map, and let $\tau : Y \to X$ be defined by $z \mapsto z^2$ where $X$, $Y$ is identified with the unit circle in the complex plane, and let $\pi_Y := \tau \circ \pi_X$. Define a sheaf $\mathscr{F}$ on $S^1$ for a nonempty open subset $U$ of $S^1$ as
        \begin{align*}
