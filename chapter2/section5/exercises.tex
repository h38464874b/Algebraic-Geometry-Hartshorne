...
\item[\textbf{1.}] Let $(X, \fO_X)$ be a ringed space, and let $\fE$ be a locally free $\fO_X$-module of finite rank. We define the \textit{dual} of $\fE$, denoted $\fE^\vee$ to be the sheaf $\rHom_{\fO_X}(\fE, \fO_X)$.
\begin{itemize}
    \item[(a)] Show that $(\fE^\vee)^\vee \cong \fE$.
    \item[(b)] For any $\fO_X$-module $\fF$, $\rHom_{\fO_X}(\fE, \fF) \cong \fE^\vee \otimes_{\fO_X} \fF$.
    \item[(c)] For any $\fO_X$-modules $\fF$, $\fG$, $\Hom_{\fO_X}(\fE \otimes \fF, \fG) \cong \Hom_{\fO_X}(\fF, \rHom_{\fO_X}(\fE, \fG)).$
...
\begin{lemma}
    Let $(X, \fO)$ be a ringed space, let $\fF, \fG$ be $\fO$-modules, and let $\fF$ be locally free of finite rank. For any $x \in X$, $\rHom(\fF, \fG)_x \cong \Hom_{\fO_{x, X}}(\fF_x, \fG_x)$.
\end{lemma}

\begin{proof}
    An element of $\rHom(\fF, \fG)_x$ can be represented as a pair $\langle U, f \rangle$, where $U$ is an open neighborhood of $x$ and $f : \restr{\fF}{U} \to \restr{\fG}{U}$ is a morphism of sheaves on $U$. Since $(\restr{\fF}{U})_x \cong \fF_x, (\restr{\fG}{U})_x \cong \fG_x$, there is a natural map
    \begin{align*}
        \alpha : \rHom(\fF, \fG)_x & \to \Hom_{\fO_{x, X}}(\fF_x, \fG_x) \\
...
\begin{itemize} [leftmargin=0em]
    ...
    \begin{align*}
        \rHom_{X}(\fE, \fF)(U) & = \Hom_{\fO_U}(\restr{\fE}{U}, \restr{\fF}{U}), \\
        (\fE^\vee \otimes_{\fO_X} \fF)(U) & = \fE^\vee(U) \otimes_{\fO_U} \fF(U) = \Hom_{\fO_U}(\restr{\fE}{U}, \restr{\fO_X}{U}) \otimes_{\fO_U} \fF(U),
    \end{align*}
    and since $\fE$ is free, we have $\fE(U) = \fO_X(U)^{\oplus n}$. Let $f_i : \fE \to \fO_X$ be the projection morphisms onto the $i$th component, let $\restr{e_i}{U}$ be the standard basis of $\fE(U)$, and let $f : \rHom_{\fO_X}(\fE, \fO_X) \to \fE^\vee \otimes_{\fO_X} \fF$ be the morphism of sheaves defined by
    \begin{align*}
        f(U) : \rHom_{X}(\fE, \fF)(U) & \to (\fE^\vee \otimes_{\fO_X} \fF)(U) \\
    ...
    \end{align*}
    We want to show $f$ naturally defines an element of $\Hom_{\fO_X}(\fF, \rHom_{\fO_X}(\fE, \fG))$. Define $\tilde{f} : \fF \to \rHom_{\fO_X}(\fE, \fG)$ as the following: for each $s \in \fF(U)$, let $\tilde{f}(s) : \restr{\fE}{U} \to \restr{\fG}{U}$ be the morphism defined by
    \begin{align*}
    ...
    \item[(d)] The question is local on $Y$, so assume $\fE$ is free of finite rank. Also, if the statement is true for two sheaves of $\fO_Y$-modules, then it true for their direct sum, because left-adjoint functors preserve colimits, so we can assume $\fE = \fO_Y$. Thus, we have the following composite of canonical isomorphisms
    \begin{equation*}
...
\item[\textbf{12.}] \begin{itemize}
    \item[(a)] Let $X$ be a scheme over a scheme $Y$, and let $\fL$, $\fM$ be two very ample invertible sheaves on $X$. Show that $\fL \otimes \fM$ is also very ample.

    \item[(b)] Let $f : X \to Y$ and $g : Y \to Z$ be two morphisms of schemes. Let $\fL$ be a very ample invertible sheaf on $X$ relative to $Y$, and let $\fM$ be a very ample invertible sheaf on $Y$ relative to $Z$. Show that $\fL \otimes f^* \fM$ is a very ample invertible sheaf on $X$ relative to $Z$.
\end{itemize}
